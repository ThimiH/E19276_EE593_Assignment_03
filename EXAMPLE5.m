%% An Application Using Simulated Signals on Newton's Method
...
%% TASK 4: Add a small random number to R^(-1) and see the effect on
%% convergence
...
while((norm(w-wp)>EPS)&&(I<MaxIter))
    I = I + 1 ;
    wp = w ;
    w = w - 2*Mu*(inv(R) + dR )*(R*w - p) ;
    W(:,end+1) = w ;
end
